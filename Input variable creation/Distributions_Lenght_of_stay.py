...
for group_index, group in enumerate(groups):
    group_data = df_train[df_train['Group'] == group]['Wardtime']
    ...
    ax_hist.set_xlabel('Length of Stay')
    ...
    ax_box.set_xlabel('Length of stay')
    ax_box.set_yticks([])  # Hide y-axis ticks

plt.tight_layout()  # Adjust layout to make labels and titles readable
plt.savefig('../Input variable creation/histogram_boxplot_Length_of_stay.png')
...
plt.tight_layout()  # Adjust layout to make labels and titles readable
plt.savefig('qq_plots_Length_of_stay.png')
plt.close()  # Close the figure after saving to release resources
...
params[['Shape_Length_of_stay', 'Loc_Length_of_stay', 'Scale_Length_of_stay']] = params['Parameters'].str.strip(
...
group_data = pd.merge(group_data, average_time_per_group)
params = pd.merge(params, group_data, on='Group', how='left')
params = params.rename({'Wardtime' : 'Avarage_Wardtime'})

# Save results to Excel files
params.to_excel('../Input_Data/Distributie_parameters_Length_of_stay.xlsx', index=False)
all_results.to_excel('Length_of_stay_test_results.xlsx')
